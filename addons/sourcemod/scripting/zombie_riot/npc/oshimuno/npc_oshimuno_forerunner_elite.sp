...
void OshimunoForerunnerEliteOnMapStart()
{
	...
	NPCData data;
	strcopy(data.Name, sizeof(data.Name), "Tarakeno Elite Forerunner");
	strcopy(data.Plugin, sizeof(data.Plugin), "npc_oshimuno_forerunner_elite");
...
{
	return OshimunoForerunnerElite(vecPos, vecAng, team);
}

methodmap OshimunoForerunnerElite < CClotBody
{
	...
	public OshimunoForerunnerElite(float vecPos[3], float vecAng[3], int ally)
	{
		OshimunoForerunnerElite npc = view_as<OshimunoForerunnerElite>(CClotBody(vecPos, vecAng, "models/player/demo.mdl", "1.0", "1000", ally));
...
{
	OshimunoForerunnerElite npc = view_as<OshimunoForerunnerElite>(iNPC);
	...
	if(target > 0)
	{
		...
		OshimunoForerunnerEliteSelfDefense(npc, distance, vecTarget, gameTime);
	}
...
void OshimunoForerunnerEliteSelfDefense(OshimunoForerunnerElite npc, float distance, float vecTarget[3], float gameTime)
{
...
{
	OshimunoForerunnerElite npc = view_as<OshimunoForerunnerElite>(entity);
